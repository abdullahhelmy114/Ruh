...
interface Course {
  ...
  level: string; price: number; image_url: string | null;
  trailer_url?: string | null; teacher_name: string; teacher_uid: string;
  lessons: Lesson[];
}
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-8">
          <div className="rounded-3xl overflow-hidden bg-linear-to-br from-emerald-600 to-emerald-800 h-64 md:h-80 flex items-center justify-center">
            {course.image_url ? <Image src={course.image_url} alt={course.title} width={800} height={400} className="object-cover w-full h-full" /> : <BookOpen className="h-24 w-24 text-white/20" />}
          </div>
          {course.trailer_url && <div className="rounded-3xl overflow-hidden shadow-elegant"><YouTubeEmbed url={course.trailer_url} title={course.title} /></div>}
          <div>
            ...
            <div className="flex items-center gap-2 mt-3 text-sm text-muted-foreground"><User size={16} /> <T>by</T> <Link href={`/teachers/${course.teacher_uid}`} className="font-medium text-foreground hover:text-amber-600">{course.teacher_name}</Link></div>
          </div>
          ...
          <div>
            ...
            {course.lessons?.length === 0 ? <p className="text-muted-foreground"><T>No lessons yet.</T></p> : course.lessons?.map(lesson => (
              <div key={lesson.id} className="border rounded-2xl p-4 bg-card mb-3">
                ...
                {lesson.recording_url && <div className="mt-3 rounded-xl overflow-hidden"><YouTubeEmbed url={lesson.recording_url} title={lesson.title} /></div>}
              </div>
            ))}
          </div>
        </div>
        <div className="space-y-4">
          <div className="rounded-3xl border bg-card p-6 shadow-elegant sticky top-24">
            ...
            <div className="mt-4 space-y-2">
              <div className="flex justify-between text-sm"><span className="text-muted-foreground"><T>Level</T></span><span className="font-medium">{course.level}</span></div>
              <div className="flex justify-between text-sm"><span className="text-muted-foreground"><T>Lessons</T></span><span className="font-medium">{course.lessons?.length || 0}</span></div>
            </div>
            {message && <div className={`mt-3 text-xs text-center font-medium ${message.includes("success") ? "text-emerald-600" : "text-red-500"}`}><T>{message}</T></div>}
            <button onClick={handleEnroll} disabled={enrolling} className="mt-4 w-full rounded-full bg-linear-to-r from-emerald-600 to-emerald-700 py-3 text-sm font-semibold text-white disabled:opacity-50 flex items-center justify-center gap-2">
              {enrolling ? <Loader2 size={16} className="animate-spin" /> : <><CreditCard size={16} /> <T>Enroll Now</T></>}
            </button>
            <Link href="/marketplace" className="mt-4 flex items-center justify-center gap-2 text-sm text-muted-foreground hover:text-foreground"><ArrowLeft size={16} /> <T>Back to Marketplace</T></Link>
          </div>
        </div>
      </div>
    </div>
