...
export async function GET(request: Request) {
  ...
  try {
    const [profile] = await sql`
      SELECT full_name, email, referral_code, referral_count, credits
      FROM profiles WHERE firebase_uid = ${uid} AND role = 'student'
    `;
    ...
    const inProgress = await sql`
      SELECT c.id AS course_id, c.title, p.progress_percent,
             (SELECT l.title FROM lessons l WHERE l.course_id = c.id ORDER BY l.order_index ASC LIMIT 1 OFFSET (FLOOR((p.progress_percent / 100.0) * (SELECT COUNT(*) FROM lessons WHERE course_id = c.id)) + 1) - 1) AS next_lesson
      FROM progress p
    ...
    const completed = await sql`
      SELECT c.title, p.completed_at
      FROM progress p
      JOIN courses c ON p.course_id = c.id
      WHERE p.user_uid = ${uid} AND p.completed = true
      ORDER BY p.completed_at DESC
    `;

    // الجلسات المباشرة
    const sessions = await sql`
      SELECT l.id, l.title, l.scheduled_at, l.meeting_url, l.course_id, c.title AS course_title, t.full_name AS teacher_name
      FROM lessons l
